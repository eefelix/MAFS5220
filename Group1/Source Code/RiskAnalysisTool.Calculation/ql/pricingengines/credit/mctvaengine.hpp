...
//! \file mctvaengine.hpp
//! \brief Pricing engine class for TVA calculation via Monte Carlo Simulation. 
//!
namespace QuantLib {
	//! \brief Pricing engine class for TVA calculation via Monte Carlo Simulation. 
	/*!	This class is the pricing engine for the "instrument" TVA, responsible for the calculation of TVA
		(includes CVA, DVA and FVA) via Monte Carlo Simulation framework.
	...
	*/
	...
	class MCTVAEngine : public TVAEngine<TVA> /*TVA::engine*/
		, public McSimulation < MultiValueMultiVariate, RNG, S >, public boost::enable_shared_from_this < MCTVAEngine<RNG,S> > {
	public:
	...
	public:
		...
		//{@
		//!	Creates an instance of MC TVA engine using two counterparties, instruments (single instrument or portfolio),
		//!	correlation matrix of the counterparties and instruments, and the yield term structure (deterministic or
		//! stochastic), these information may be got from the "instrument" TVA. 
		MCTVAEngine(
			...
			) : TVAEngine(endTime, counterparties, instrumentModels, domesticCurve, domesticCIRModel), McSimulation(antitheticVariate, false),
	...
	public:
		...
		void calculate() const {
			...
			//results_.DVA = mean[1];
			results_.FVA = mean[0];
			results_.value = /*results_.CVA - results_.DVA - */results_.FVA;

			if (RNG::allowsErrorEstimate) {
				results_.errorEstimate = errorEstimate[0]/* + errorEstimate[1] + errorEstimate[2]*/;
			}
		}
...
*/
	...
	protected:
		...
		virtual boost::shared_ptr<path_pricer_type> pathPricer() const {
			return boost::make_shared<TVAPathPricer<MCTVAEngine<RNG,S>>>(shared_from_this());
		}
		...
		/*virtual std::vector<boost::shared_ptr<StochasticProcess1D>> instrumentProcess() const{
			std::vector<boost::shared_ptr<StochasticProcess1D>> processes;
			for (boost::shared_ptr<const MCExposureModel> model : tvaModels_)
	...
	private:
		...
		mutable std::map<boost::shared_ptr<StochasticProcess1D>, int> processToPath_;
