...
namespace QuantLib
{
	void TestFVACalculation(
		...
		boost::shared_ptr<MCTVAEngine<>> mcTVAEngine = boost::make_shared<MCTVAEngine<>>(
			...
			false, 500, Null<Real>(), 50000, dayCounter.yearFraction(referenceDate, maturityDate));
		...
		Assert::AreEqual(expectedValue, fva, 0.1 * expectedValue);
	};

	TEST_CLASS(IRSFVATest)
	{
	public:
		TEST_METHOD(IRSFVATestSpreadSheet)
		{
			Calendar calendar = TARGET();
			DayCounter dayCounter = Actual360();
			...
			TestFVACalculation(
				...
				dates, discount, cdsspreads1, tenor1,
				cdsspreads2, tenor2, fixedSchedule, floatSchedule,
				nominal, fixedRate, spread, swapType, -4000);
		}
	};
}